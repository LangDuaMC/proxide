...
class MetricsRegistry(val core: Core, val globalTags: Iterable<Tag>?, val hookRegistry: ExpositionRegistry) {
    private val registry = PrometheusMeterRegistry(PromConfig)
    private var server: EmbeddedServer<*, *>? = null

    fun onEnable() {
        server = embeddedServer(Netty, port = core.config.endpointPort, host = core.config.endpointHost) {
            install(Compression) {
                gzip {
            ...
            install(MicrometerMetrics) {
                this.registry = this@MetricsRegistry.registry
            ...
            routing {
                get("/metrics") {
                    call.respond(buildString {
                        append(this@MetricsRegistry.registry.scrape())
